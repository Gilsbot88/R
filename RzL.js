...
no jual sc ini !!
sc ini gratis !!
ᴅᴇɴɢᴇʀ ᴛᴜʜ ᴋᴀᴛᴀ sɪ ᴅᴀɴɪ ᴊɢɴ ɴɢᴀᴋᴜ𝟸
ᴍᴀᴋᴇ ᴛɪɴɢɢᴀʟ ᴍᴀᴋᴇ ᴊɢɴ ʙᴀɴʏᴀᴋ ᴛɪɴɢᴋᴀʜ 
ᴄᴏᴘʏʀɪɢʜᴛ ʙʏ ʀᴢʟᴍᴏᴅᴢ
...
gaya1 = setting.gaya1
gaya2 = setting.gaya2
const fakeimage = fs.readFileSync ('./media/RzL.jpg')
...
module.exports = RzL = async (RzL, mek, _welkom) => {
	try {
		...
		const botNumber = RzL.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6281333603591@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? RzL.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const groupMetadata = isGroup ? await RzL.groupMetadata(from) : ''.toString
        ...
        m = simple.smsg(RzL, mek)
        var pes = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
        const messagesD = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
		const conts = mek.key.fromMe ? RzL.user.jid : RzL.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? RzL.user.name : conts.notify || conts.vname || conts.name || '-'    
		...
		let bio_nya = await RzL.getStatus(sender)
		try {
			...
			try {
		pporang = await RzL.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
success: 'Sukses tod✔️️',
error: {
stick: 'Itu bukan sticker cok !!',
Iv: 'Link sesad !!'
},
only: {
...
RzL.sendMessage(from, buttonMessages, buttonsMessage, {
quoted: fgi
})
}
...
RzL.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await RzL.prepareMessage(from, kma, image)
...
RzL.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await RzL.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await RzL.prepareMessage(from, kma, location)
const buttonMessages = {
...
let po = RzL.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "𝗠𝗘𝗡𝗨","footerText": `${tanggal}`,"listType": "SINGLE_SELECT","sections": list}}, {})
return RzL.relayWAMessage(po, {waitForAck: true})
}
const reply = (teks) => {
RzL.sendMessage(from, teks, text, {quoted:mek})
}
const sendMess = (hehe, teks) => {
RzL.sendMessage(hehe, teks, text)
}
const isUrl = (url) => {
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
        }
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? RzL.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : RzL.sendMessage(from, teks.trim(), extendedText, { quoted: fgi, contextInfo: { "mentionedJid": memberr } })
}
const costum = (pesan, tipe, target, target2) => {
RzL.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
}
...
RzL.sendMessage(to, media, type, { quoted: fgi, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
const daftar1 = `Hai tod  ${pushname} ${ucapanWaktu} \n\nSebelum Mengakses Bot Verify Terlebih Dahulu Ya `
const daftar2 = '```Ketik Tombol Di Bawah Untuk Verify Tod\nJika Tidak Support Button Ketik #verify```'
...
let anu = await RzL.groupMetadata(from)
let members = anu.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
RzL.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/RzL.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}
...
RzL.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
}
const sendKontak = (from, nomor, nama) => {
const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer ${botname}\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
RzL.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
}
...
reply(` *「 GROUP LINK DETECTOR 」*\nTerdeteksi mengirimkan link grup bokep, out lu ngentod!!!`)
setTimeout(() => {
RzL.groupRemove(from, [kic]).catch((e) => { reply(`BOT NOT ADMIN`) })
}, 0)
}
...
reply(`「 *VIRTEX DETECTOR* 」\n\nKamu mengirimkan virtex, out lu cil!!!`)
console.log(color('[KICK]', 'red'), color('Received a virus text!', 'yellow'))
RzL.groupRemove(from, [sender])
}     
if (!m.key.fromMe && !isOwner && bad.includes(messagesD)) {
ano = fs.readFileSync('./media/sticker/toxic.webp')
RzL.sendMessage(from, ano, sticker, { quoted: mek})
}

//══════════[ Dll ]══════════//

if (autoread){
RzL.chatRead(from, "read")
} else if (autoketik) {
RzL.updatePresence(from, Presence.composing)
} else if (autovn) {
RzL.updatePresence(from, Presence.recording)
}
...
const { wa_version, os_version } = RzL.user.phone
...
${gaya1} *Wa Version :* ${RzL.user.phone.wa_version}
...
RzL.sendMessage(from, sound, audio, { mimetype: 'audio/mp4', ptt: true })
break
case 'command':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
listMsg = {
...
RzL.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [senderr]},quoted:fgi})
break
case 'allmenu':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'ʙᴀᴄᴋ' }, type: 1 },{ buttonId: `${prefix}abcde`, buttonText: { displayText: '\n\nAKU ADALAH ANAK NGENTOT' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'downloadmenu':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
menu =
...
case 'abcde':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply('njirr, emang bener sih _Mengaku dengan gaya_\nLu kan anak hasil ngentot awokawok\nCanda tod jangan baperan,baper ?mati ae 🗿')
break
...
• Dana : _083136890848_
• Qris All pay : _Scan nya eror langsung aja konfirmasi ke owner_
...
url = args.join(' ')
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/download/ytmp3?url=${url}&apikey=${dhakey}`)
ytmp3 = await getBuffer(anu.result.url)
reply(`_audio sedang diproses, silahkan tunggu beberapa saat lagi_`)
RzL.sendMessage(from, ytmp3, audio, {mimetype:"audio/mp4", quoted:mek})
break
case 'ytmp4':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply("Masukkan url youtube")
url = args.join(' ')
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/download/ytmp4?url=${url}&apikey=${dhakey}`)
ytmp4 = await getBuffer(anu.result.url)
reply(`_video sedang diproses, silahkan tunggu beberapa saat lagi_`)
RzL.sendMessage(from, ytmp4, video, {caption:`Done kak`, thumbnail:Buffer.alloc(0), quoted:mek})
break
case 'tiktok':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply("Masukkan url tiktok")
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/download/tiktok?url=${q}&apikey=${dhakey}`)
tiktok = await getBuffer(anu.result.nowatermark)
RzL.sendMessage(from, tiktok, video, {quoted: mek, caption : 'Done bro'})
break
case 'tiktokwm':
case 'ttwm':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendmessage(from, tiktok, audio, {quoted: mek})
break
...
case 'nantalia':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/asupan/${command}?apikey=${dhakey}`)
...
case 'hijaber':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/asupan/hijaber?apikey=${dhakey}`)
text1 = `Klik next untuk ke ${command} selanjutnya`
text2 = `Mohon untuk tidak spam !!`
but = [{ buttonId: `${prefix+command}`, buttonText: { displayText: '️NEXT ➡️' }, type: 1 }]
...
case 'malaysia':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/cecan/${command}?apikey=${dhakey}`)
text1 = `Klik next untuk ke ${command} selanjutnya`
text2 = `Mohon untuk tidak spam !!`
but = [{ buttonId: `${prefix+command}`, buttonText: { displayText: '️NEXT ➡️' }, type: 1 }]
sendButImage(from, text1, text2, buffer, but, {quoted: mek})
...
case 'dare':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/randomtext/${command}?apikey=${dhakey}`)
buffer = (anu.result)
...
case 'quotes':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/randomtext/quotes?apikey=${dhakey}`)
...
case 'quoteskanye':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/randomtext/quotes/kanye?apikey=${dhakey}`)
...
case 'ppcouple':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/randomimage/ppcouple?apikey=${dhakey}`)
...
case 'meme':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/randomimage/meme?apikey=${dhakey}`)
...
case 'wallpapergame':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/randomimage/${command}?apikey=${dhakey}`)
...
case 'darkjokers':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/randomimage/darkjokes?apikey=${dhakey}`)
text1 = `Klik next untuk ke ${command} selanjutnya`
text2 = `Mohon untuk tidak spam !!`
but = [{ buttonId: `${prefix+command}`, buttonText: { displayText: '️NEXT ➡️' }, type: 1 }]
sendButImage(from, text1, text2, buffer, but, {quoted: mek})
...
case 'nino':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/anime/${command}?apikey=${dhakey}`)
text1 = `Klik next untuk ke ${command} selanjutnya`
text2 = `Mohon untuk tidak spam !!`
but = [{ buttonId: `${prefix+command}`, buttonText: { displayText: '️NEXT ➡️' }, type: 1 }]
sendButImage(from, text1, text2, buffer, but, {quoted: mek})
...
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/nsfw/${command}?apikey=${dhakey}`)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
reply(mess.wait)
buffer = await getBuffer(`https://apiRzL.herokuapp.com/api/randomvideo/${command}?apikey=${dhakey}`)
text1 = `Klik next untuk ke ${command} selanjutnya`
text2 = `Mohon untuk tidak spam !!`
but = [{ buttonId: `${prefix+command}`, buttonText: { displayText: '️NEXT ➡️' }, type: 1 }]
sendButVideo(from, text1, text2, buffer, but, {quoted: mek})
...
teks = args.join(" ")
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/${command}?apikey=${dhakey}&text=${teks}`)
oke = await getBuffer(anu.result)
RzL.sendMessage(from, oke, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
case 'hartatahta': 
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Teks nya mana?*\n_Contoh : ${prefix + command} namamu_`) 
teks = args.join(" ")
reply(mess.wait)
harta = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/harta-tahta?apikey=${dhakey}&text=${teks}`)
RzL.sendMessage(from, harta, image, {quoted: mek, caption: '*Nih Kak Dah Jadi*'})
break
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/8bit?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'pornhub':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Contoh : ${prefix + command} nama1&nama2*`)
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/pornhub?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'glitch':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/glitch?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'glitch2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/glitch2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'layered':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/layered?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case '3dsteel':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/3dsteel?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'realistic':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/realistic?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'lionlogo':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/lionlogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'ninjalogo':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/ninjalogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'wolf':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/logowolf?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'wolf2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/logowolf2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'halloween3':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/halloween3?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'marvel':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/marvelstudio?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'marvel2':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/marvelstudio2?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/cinematichorror?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'avengers':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/avengerslogo?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'graffiti3':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/coolwallgraffiti?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'captainamerica':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
anu = await fetchJson(`https://apiRzL.herokuapp.com/api/textpro/captainamerica?apikey=${dhakey}&text1=${F1}&text2=${F2}`)
pornhub = await getBuffer(anu.result)
RzL.sendMessage(from, pornhub, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'girlneko':
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/girlneko?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'sadboy':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (args.length < 1) return reply(`*Contoh : ${prefix + command} nama1&nama2*`)
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/sadboy?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'makerkaneki':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/kaneki?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'rem':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/rem?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'lolimaker':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/lolimaker?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
case 'gura':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
var F2 = F.split("&")[1]; 
reply(mess.wait)
girlneko = await getBuffer(`https://apiRzL.herokuapp.com/api/maker/gura?apikey=${dhakey}&text=${F1}&text2=${F2}`)
RzL.sendMessage(from, girlneko, image, {caption: `*Nih kak dah jadi*`, quoted: mek})
break
...
RzL.sendMessage(from, buff, image, {quoted: mek, caption : teks})
break
case 'sc':
case 'script':
case 'sourcode':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
fakeitem(`_Silakan mampir ke channel ini_\n_Karena disana ada script bot keren²_\n_Gak mampir sangat rugi besar_\n_https://youtube.com/channel/UCJi3qEvIX7AZyRowqSTnoig_`)
break
case 'rules':
case 's&k':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
case 'delete':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
RzL.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
break
case 'verify':
case 'daftar':
if (isRegistered) return textImg('_sᴏʀʀʏ ᴛᴏᴅ ʟᴜ ᴍᴀsɪʜ ᴋᴇᴄɪʟ ʙʟᴍ ᴄᴜᴋᴜᴘ ᴜᴍᴜʀ_')
const serialUser = createSerial(18)
veri = sender
_registered.push(sender)
fs.writeFileSync('./database/registered.json', JSON.stringify(_registered))
addRegisteredUser(sender, serialUser)
const jancok = `「 *PERETASAN AKUN* 」
...
mhan = await RzL.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const sendBtnVeryy = {
imageMessage: mhan.message.imageMessage,
contentText:`${jancok}`,
footerText:'Terimakasih Nomor Anda sebentar lagi akan diban oleh WhatsApp', 
buttons: gbutsan,
headerType: 4
}
RzL.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:fgi, contextInfo: { mentionedJid: [sender]}})
...
RzL.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
break
case 'kapankah':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
break
case 'bisakah':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
break
case 'bagaimanakah':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.sendMessage(from, 'Pertanyaan : *'+bagaimanakah+'*\n\nJawaban : '+ mana, text, { quoted: mek })
break
case 'belah':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.sendMessage(from, 'Pertanyaan : *bener atau salah '+iyas+'*\n\nJawaban : '+ salah, text, { quoted: mek })
break
...
RzL.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
break
case 'ceksange':
case 'sangecek':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, 'Pertanyaan : *'+sange+'*\n\nJawaban : '+ nge+'%', text, { quoted: mek })
break
case 'cekgay':
case 'gaycek':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, 'Pertanyaan : *'+gayy+'*\n\nJawaban : '+ yag+'%', text, { quoted: mek })
break
case 'gantengcek':
case 'cekganteng':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
break
case 'cantikcek':
case 'cekcantik':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
break
case 'cekwatak':
case 'watakcek':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: mek })
break
case 'cekhobby':
case 'hobbycek':
case 'cekhoby':
...
RzL.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
break
case 'ceklesbi':
case 'lesbicek':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
RzL.sendMessage(from, 'Pertanyaan : *'+lesbii+'*\n\nJawaban : '+ bi+'%', text, { quoted: mek })
break
case 'cekme':
case 'me':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
try {
profil = await RzL.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
RzL.sendMessage(from, buff, image, {quoted: mek, caption: profile, contextInfo: { mentionedJid: [bet]}})
break
case 'cekmati':
case 'mati':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
const media = await RzL.downloadAndSaveMediaMessage(encmedia)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
...
.on('end', function () {
console.log('Finish')
RzL.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
media = await RzL.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Gagal, pada saat mengkonversi sticker ke gambar')
buffer = fs.readFileSync(ran)
RzL.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
fs.unlinkSync(ran)
})
break
...
var media = await RzL.downloadAndSaveMediaMessage(enmedia)
var njay = await imgbb('520bd6f6209077d1777c2a4f20c509c2', media)
var resu = await getBuffer(`https://api.memegen.link/images/custom/${teks1}/${teks2}.png?background=${njay.display_url}`)
RzL.sendMessage(from, resu, image, {quoted: mek})
fs.unlinkSync(media)
} catch (e) {
return reply(`${e}`)
console.log(e)
...
+ 'END:VCARD'.trim()
RzL.sendMessage(from, {displayName: `Ownernya ${botname}`, vcard: vcard2}, contact, 
{ quoted: fgi, 
})
...
anu = await RzL.chats.all()
if (isMedia && !RzL.message.videoMessage || isQuotedImage) {
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
buff = await RzL.downloadMediaMessage(encmedia)
for (let _ of anu) {
RzL.sendMessage(_.jid, buff, image, { caption: `${body.slice(7)}` })
}
...
case 'bc':
             ...
             anu100 = await RzL.chats.all()
             if (isMedia && !RzL.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(RzL).replace('quotedM','m')).message.extendedTextMessage.contextInfo : RzL
             bc100 = await RzL.downloadMediaMessage(encmedia)
             for (let _ of anu100) {
             RzL.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu100) {
             RzL.sendMessage(_.jid, 
			{"contentText": `*「 PESAN SIARAN BOT TOD 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
buff = await RzL.downloadMediaMessage(encmedia)
for (let _ of anu) {
RzL.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
}
...
buff = await RzL.downloadMediaMessage(encmedia)
for (let _ of anu) {
RzL.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
}
reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
buff = await RzL.downloadMediaMessage(encmedia)
for (let _ of anu) {
RzL.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
}
reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
} else {
for (let _ of anu) {
...
if (isMedia && !mek.message.videoMessage || isQuotedImage) {
const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
bcgc = await RzL.downloadMediaMessage(encmedia)
for (let _ of groupMembers) {
RzL.sendMessage(_.jid, bcgc, image, { caption: `*「 BROADCAST GROUP 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
...
if (!isOwner && !mek.key.fromMe) return  reply(mess.only.owner)
anu = await RzL.chats.all()
RzL.setMaxListeners(25)
for (let _ of anu) {
RzL.deleteChat(_.jid)
}
...
media = await RzL.downloadAndSaveMediaMessage(encmedia, './database/sampah/media_user')
await RzL.updateProfilePicture(botNumber, media)
...
RzL.sendMessage(from, argzi[0], MessageType.text)
}
break
...
if (args[0] === 'on') {
if (isNsfw) return reply('ʀᴇᴀᴅʏ ᴅɪɢᴜɴᴀᴋᴀɴ')
_nsfw.push(from)
...
if (!isNsfw) return reply('sᴏʀʀʏ ᴇʀʀᴏʀ ᴀᴡᴏᴋᴀᴡᴏᴋ ᴅʜ ᴍᴀᴛɪ')
...
await RzL.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: fgi})
}
break
case 'welcome': 
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
if (args[0] === 'on') {
if (isWelkom) return reply('ʀᴇᴀᴅʏ ᴅɪɢᴜɴᴀᴋᴀɴ')
_welkom.push(from)
fs.writeFileSync('./database/welcome.json', JSON.stringify(_welkom))
reply(`\`\`\`Sukses ✅, Mengaktifkan fitur welcome di group\`\`\` *${groupMetadata.subject}*`)
} else if (args[0] === 'off') {
if (!isWelkom) return reply('sᴏʀʀʏ ᴇʀʀᴏʀ ᴀᴡᴏᴋᴀᴡᴏᴋ ᴅʜ ᴍᴀᴛɪ')
...
if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
if (args[0] === 'on') {
if (isAntiLink) return reply('ʀᴇᴀᴅʏ ᴅɪɢᴜɴᴀᴋᴀɴ')
_antilink.push(from)
fs.writeFileSync('./database/antilink.json', JSON.stringify(_antilink))
reply(`\`\`\`Sukses ✅, Mengaktifkan fitur antilink di group\`\`\` *${groupMetadata.subject}*`)
} else if (args[0] === 'off') {
if (!isAntiLink) return reply('sᴏʀʀʏ ᴇʀʀᴏʀ ᴀᴡᴏᴋᴀᴡᴏᴋ ᴅʜ ᴍᴀᴛɪ')
...
if (!isGroupAdmins && !mek.key.fromMe) return reply(mess.only.admin)
if (args[0] === 'on') {
if (isAntiVirtex) return reply('ʀᴇᴀᴅʏ ᴅɪɢᴜɴᴀᴋᴀɴ')
_antivirtex.push(from)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(_antivirtex))
reply(`\`\`\`Sukses ✅, Mengaktifkan fitur antivirtex di group\`\`\` *${groupMetadata.subject}*`)
} else if (args[0] === 'off') {
if (!isAntiVirtex) return reply('sᴏʀʀʏ ᴇʀʀᴏʀ ᴀᴡᴏᴋᴀᴡᴏᴋ ᴅʜ ᴍᴀᴛɪ')
...
RzL.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'tutup') {
reply(`*Berhasil Memtutup Grup ${groupMetadata.subject}*`)
RzL.groupSettingChange(from, GroupSettingChange.messageSend, true)
}
break
case 'linkgroup':
case 'linkgrup':
...
if (!isGroup) return reply(mess.only.group)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
linkgc = await RzL.groupInviteCode(from)
yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
RzL.sendMessage(from, yeh, text, { quoted: fgi })
break
case 'promote' :
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.groupMakeAdmin(from, mentioned)
} else {
...
mentions(teks, mentioned, true)
RzL.groupDemoteAdmin(from, mentioned)
} else {
...
RzL.groupDemoteAdmin(from, members_id)
break
case 'promoteall':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
RzL.groupMakeAdmin(from, members_id)
break
case 'add' :
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid;
if (mentioned.length > 1) {
RzL.groupRemove(from, mentioned);
reply(mess.success);
} else if (mentioned.length < 1) {
anu = mek.message.extendedTextMessage.contextInfo.participant;
RzL.groupRemove(from, [anu]);
reply(mess.success);
} else {
RzL.groupRemove(from, mentioned);
reply(mess.success);
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
RzL.groupUpdateSubject(from, `${body.slice(9)}`)
RzL.sendMessage(from, `\`\`\`Sukses ✅, Mengganti nama grup menjadi\`\`\` *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setdesc':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
RzL.groupUpdateDescription(from, `${body.slice(9)}`)
RzL.sendMessage(from, `\`\`\`Sukses ✅, Mengganti deskripsi grup\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setppgrup':
case 'setpp':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
...
let media = await RzL.downloadMediaMessage(encmedia)
RzL.updateProfilePicture(from, media)
...
if (!isGroup) return reply(mess.only.group)
try {
var pic = await RzL.getProfilePicture(from)
} catch {
var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n\n${groupMetadata.desc}`
RzL.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
break
case 'resetlinkgc':
case 'resetlinkgroup':
case 'resetlinkgrup':
...
let online = [...Object.keys(RzL.chats.get(ido).presences), RzL.user.jid]
RzL.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
} catch (e) {
reply(`${e}`)
}
break
...
file = await RzL.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
value = args.join(" ")
var group = await RzL.groupMetadata(from)
...
RzL.sendMessage(from, ini_buffer, sticker, options)
fs.unlinkSync(file)
} else {
...
ini_buffer = fs.readFileSync(file)
RzL.sendMessage(from, ini_buffer, image, options)
fs.unlinkSync(file)
...
ini_buffer = fs.readFileSync(file)
RzL.sendMessage(from, ini_buffer, audio, options)
fs.unlinkSync(file)
...
ini_buffer = fs.readFileSync(file)
RzL.sendMessage(from, ini_buffer, video, options)
fs.unlinkSync(file)
} else{
...
RzL.sendMessage(from, options, text, { quoted: mek })
break
case 'kontag':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgi})
if (!isGroup) return reply(mess.only.group)
